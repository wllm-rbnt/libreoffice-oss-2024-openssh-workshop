...
gateway_pub=$(sudo docker network inspect docker_public_net | grep -A 3 docker-gateway- | grep IPv4Address | cut -d \" -f 4 | cut -d / -f 1)
gateway_priv=$(sudo docker network inspect docker_private_net | grep -A 3 docker-gateway- | grep IPv4Address | cut -d \" -f 4 | cut -d / -f 1)
internal_priv=$(sudo docker network inspect docker_private_net | grep -A 3 docker-internal- | grep IPv4Address | cut -d \" -f 4 | cut -d / -f 1)
